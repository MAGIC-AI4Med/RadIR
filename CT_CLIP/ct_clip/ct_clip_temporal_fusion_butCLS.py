...
class CTCLIP_Temporal_Fusion_butCLS(nn.Module):
    def __init__(
            self,
            *,
            use_triplet_loss = 1,
    ...
    def triplet_loss(self, pred_sim, true_sim, margin=0.2, gt_threshold=0.3, positive_threshold=0.7, negative_threshold=0.4, temp=0.07):
        ...
        # 返回batch平均损失
        valid_mask = num_pairs_per_query != 0   # b n
        valid_count = valid_mask.float().sum(dim=1) # b # n个sample中有几个loss不为0
        avg_valid_loss = torch.where(
            valid_count > 0,
            (avg_loss_per_query * valid_mask.float()).sum(dim=1) / valid_count, # 这个anatomy的所有n个sample中“不为0”的loss的均值
            torch.zeros_like(valid_count) # 如果这个anatomy的所有n个sample全都为0（没有有效对），则这个anatomy的loss为0
        )
        
        # DEBUG
        if torch.isnan(avg_valid_loss).any().item():
            print('valid mask:', torch.sum(mask).item())
            print('pred_sim:', torch.max(pred_sim).item(), torch.min(pred_sim).item(), torch.isnan(pred_sim).any().item())
            print('true_sim:', torch.max(true_sim).item(), torch.min(true_sim).item(), torch.isnan(true_sim).any().item())
            print('losses:', torch.max(losses).item(), torch.min(losses).item(), torch.isnan(losses).any().item())
            print('avg_valid_loss:', torch.max(avg_valid_loss).item(), torch.min(avg_valid_loss).item(), torch.isnan(avg_valid_loss).any().item())
            raise ValueError('avg_valid_loss has nan')
        # DEBUG
    ...
    def forward(
        ...
        # DEBUG
        if torch.isnan(text_embeds).any().item():
            print('(before ViT) text_embeds:', torch.max(text_embeds).item(), torch.min(text_embeds).item(), torch.isnan(text_embeds).any().item())
            raise ValueError('text_embeds has nan')
        # DEBUG

        text_latents = self.to_text_latent(text_embeds) # B 512
        
        # NOTE: need to flatten B local_B ahead
        B, local_B, _, _, _, _ = image.shape
        image = rearrange(image, 'B N O D H W -> (B N) O D H W')
        text_latents = repeat(text_latents, 'b dim -> (b n) one dim', one=1, n=local_B) # B*N 1 512
        # NOTE
        
        # DEBUG
        if torch.isnan(text_latents).any().item():
            print('(before ViT) text_latents:', torch.max(text_latents).item(), torch.min(text_latents).item(), torch.isnan(text_latents).any().item())
            raise ValueError('text_latents has nan')
        # DEBUG
        
        _, text_latents = self.visual_transformer(image, text_latents, return_encoded_tokens=True)   
        # B*N 24 24 24 512(vis_dim) # B 1 H W dim
        
        text_latents = text_latents[:, 0, :, :, :]  # B H W dim
        
        text_latents = rearrange(text_latents, '(b n) h w dim -> b n (h w dim)', n=local_B) # B N 512
        
        # DEBUG
        if torch.isnan(text_latents).any().item():
            print('text_latents:', torch.max(text_latents).item(), torch.min(text_latents).item(), torch.isnan(text_latents).any().item())
            raise ValueError('text_latents has nan')
        # DEBUG
        
        fused_latents = self.to_visual_latent(text_latents) # B N 512
        
        fused_latents = F.normalize(fused_latents, dim = -1) # B*N 512
        
        # calculate another set of latents for image to text (vs text to image)
        # proposed by CLOOB

        if return_latents:
            return text_latents, None, fused_latents, self.temperature.exp()
        
        # DEBUG
        if torch.isnan(fused_latents).any().item():
            print('fused_latents:', torch.max(fused_latents).item(), torch.min(fused_latents).item(), torch.isnan(fused_latents).any().item())
            raise ValueError('fused_latents has nan')
        # DEBUG
        
        # pred similarity matrix
        # DEBUG
        pred_similarity_matrix = einsum('b n d, k m d -> b k n m', (fused_latents, fused_latents))
        # DEBUG
        ...
        # assert torch.max(pred_similarity_matrix).item() <=1 and torch.min(pred_similarity_matrix).item() >= -1, f'pred_similarity_matrix max {torch.max(pred_similarity_matrix).item()} or min {torch.min(pred_similarity_matrix).item()} beyond range'
        assert torch.max(gt_similarity_matrix).item() <=1 and torch.min(gt_similarity_matrix).item() >= 0, f'gt_similarity_matrix max {torch.max(gt_similarity_matrix).item()} or min {torch.min(gt_similarity_matrix).item()} beyond range'
        ...
        if self.use_infoNCE_loss > 0:
            # NOTE: temperature 使用了learnable参数
...
if __name__ == '__main__':
    ...
    video = torch.ones(size=(2, 1, 240, 480, 480)).to(device)
    ...
    text_latents, image_latents, enc_image_send = clip(text_tokens, video, return_latents=True, device=device)
