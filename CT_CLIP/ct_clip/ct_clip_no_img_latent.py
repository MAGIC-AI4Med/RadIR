...
class CTCLIP_NO_IMG_LATENT(nn.Module):
    def __init__(
            self,
            *,
            use_triplet_loss = 1,
    ...
    def forward(
        ...
        # image_latents = self.to_visual_latent(image_embeds) # 24 B*N 512  # NOTE: No Latent!
        ...
        pos = repeat(self.pos_embedding, 'one h w d dim -> (h w d) (one bn) dim', bn=text_latents.shape[1]).to(device)  # (24*24*24), B*N, 512
        
        image_latents = rearrange(enc_image, 'bn h w d dim -> (h w d) bn dim') # (24*24*24), B*N, 512
