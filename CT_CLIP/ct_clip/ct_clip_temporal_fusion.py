...
class CTCLIP_Temporal_Fusion(nn.Module):
    def __init__(
            self,
            *,
            use_triplet_loss = 1,
    ...
    def forward(
        ...
        enc_image, text_latents = self.visual_transformer(image, text_latents, return_encoded_tokens=True)   # B*N 24 24 24 512(vis_dim)

        global h_r, w_r, z_r
        h_r, w_r, z_r = enc_image.shape[1], enc_image.shape[2], enc_image.shape[3]

        enc_image = torch.mean(enc_image, dim=1)    # BN 24 24 512 (avg over z-axis)
        image_embeds = rearrange(enc_image, 'bn h w dim -> bn (h w dim)')   # B*N, 24*24*512
        
        image_latents = self.to_visual_latent(image_embeds) # B*N, 512
        
        fused_latents = rearrange(image_latents, '(b n) dim -> b n dim', n=local_B)
