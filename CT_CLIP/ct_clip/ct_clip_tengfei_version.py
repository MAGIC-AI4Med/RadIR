...
# (aligned with tengfei) Transformer Decoder
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None):
        ...
        tgt2 = self.encoder_attention(tgt, memory, memory, attn_mask=memory_mask)[0]
...
class TransformerDecoder(nn.Module):
    def __init__(self, d_model, nhead, num_layers, max_seq_len=24):
        super(TransformerDecoder, self).__init__()
        self.d_model = d_model
        self.num_layers = num_layers
        self.positional_encoding = self.get_positional_encoding(max_seq_len).permute(1,0,2)
        self.decoder_layers = nn.ModuleList([
            TransformerDecoderLayer(d_model, nhead) for _ in range(num_layers)
        ])
        self.fc_out = nn.Linear(d_model, 1)  # Example output projection layer
        self.init_weights()
        
    def get_positional_encoding(self, max_seq_len):
        # 创建一个7x7的二维位置编码
        position = torch.arange(0, max_seq_len).unsqueeze(1)  # (49, 1)
        div_term = torch.exp(torch.arange(0, self.d_model, 2).float() * -(torch.log(torch.tensor(10000.0)) / self.d_model))  # shape (256,)
        pe = torch.zeros(max_seq_len, self.d_model)  # shape (49, 512)
        pe[:, 0::2] = torch.sin(position.float() * div_term)
        pe[:, 1::2] = torch.cos(position.float() * div_term)
        pe = pe.unsqueeze(0)  # Add batch dimension, shape (1, 49, 512)
        return pe

    def init_weights(self):
        # 初始化位置编码，位置编码通常初始化为固定值
        self.positional_encoding.data.normal_(0.0, 0.1)

        # 初始化解码器层的权重
        for layer in self.decoder_layers:
            # 初始化多头注意力权重
            init.xavier_uniform_(layer.self_attention.in_proj_weight)
            init.xavier_uniform_(layer.encoder_attention.in_proj_weight)
            init.xavier_uniform_(layer.self_attention.out_proj.weight)
            init.xavier_uniform_(layer.encoder_attention.out_proj.weight)

            # 初始化前馈网络的权重
            init.xavier_uniform_(layer.ffn[0].weight)
            init.xavier_uniform_(layer.ffn[2].weight)

            # 偏置初始化为零
            if layer.ffn[0].bias is not None:
                init.zeros_(layer.ffn[0].bias)
            if layer.ffn[2].bias is not None:
                init.zeros_(layer.ffn[2].bias)

            # 初始化LayerNorm参数
            init.ones_(layer.layer_norm1.weight)
            init.zeros_(layer.layer_norm1.bias)
            init.ones_(layer.layer_norm2.weight)
            init.zeros_(layer.layer_norm2.bias)
            init.ones_(layer.layer_norm3.weight)
            init.zeros_(layer.layer_norm3.bias)
    ...
    def forward(self, text_embedding, image_embedding):
        # text_embedding: (1, bs, 512)
        # image_embedding: (24, bs, 512)
        
        bs = text_embedding.shape[1]  # batch size

        # 将位置编码加到 image_embedding 上
        image_embedding = image_embedding + repeat(self.positional_encoding, 'n one d -> n (one bs) d', bs=bs).to(image_embedding.device)  # 24 bs 512

        # 这里假设 text_embedding 是目标序列，image_embedding 是编码器的输出
        tgt = text_embedding  # (1, bs, 512) -> 转为(1, bs, 512)
        memory = image_embedding  # (24, bs, 512)
        ...
        # 输出结果
        # output = self.fc_out(tgt)  # (1, bs, 1)
        return tgt

# main clip class

class CTCLIP_Tengfei_CrossAttn(nn.Module):
    def __init__(
            self,
            *,
            use_triplet_loss = 1,
        ...
        self.fusion_module = TransformerDecoder(512, 8, 6, max_seq_len=24)
        # NOTE
    ...
    def forward(
        ...
        B, local_B, _, H, W, D = image.shape
        image = rearrange(image, 'B N O H W D -> (B N) O H W D')
        # NOTE
        ...
        fused_latents = self.fusion_module(text_latents, image_latents) # 1 B*N 512
        fused_latents = l2norm(fused_latents[0]) # B*N 512
