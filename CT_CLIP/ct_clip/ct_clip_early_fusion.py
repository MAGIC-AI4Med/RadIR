...
class CTCLIP_Earyly_Fusion(nn.Module):
    def __init__(
            self,
            *,
            use_triplet_loss = 1,
    ...
    def forward(
        ...
        B, local_B, O, D, H, W = image.shape
        image = rearrange(image, 'B N O D H W -> (B N) O D H W')
        # NOTE
        
        # project image to latents
        text_embeds = enc_text[:,0,:] # B 768
        text_latents = self.to_text_latent(text_embeds) # B 512
        text_latents = repeat(text_latents, 'b dim -> (b n) one dim', n=local_B, one=1)    # B*N 1 512
        
        fused_latents = self.visual_transformer(image, text_latents, return_encoded_tokens=True)[:, 0, :]   # B*N 1 512 -> B*N 512

        # project image to latents
        fused_latents = l2norm(fused_latents) # B*N 512
        
        fused_latents = rearrange(fused_latents, '(b n) dim -> b n dim', b=B)

        # calculate another set of latents for image to text (vs text to image)
        # proposed by CLOOB

        if return_latents:
            return None, None, fused_latents, self.temperature.exp()
...
if __name__ == '__main__':
    ...
    from transformer_maskgit import CTViT_Text_Fusion
    ...
    video = torch.ones(size=(2, 3, 1, 240, 480, 480)).to(device)
    gt_similarity_matrix = torch.ones((2, 3, 3)).to(device)
    ...
    with torch.no_grad():
        ...
        loss = clip(text_tokens, video, gt_similarity_matrix=gt_similarity_matrix, return_loss=True, device=device)
        text_latents, image_latents, fused_latents, enc_image_send = clip(text_tokens, video, return_latents=True, device=device)
        print(loss, fused_latents.shape)