...
class Attention(nn.Module):
    ...
    def forward(
        ...
        q = q * self.q_scale
        k = k * self.k_scale
        # # 检测q和k的shape是否正确，并检查是否存在NAN,inf的情况
        # # Debug checks for shape and NaN/inf values
        ...
        assert q.shape[1] == k.shape[1], f"Head count mismatch: {q.shape[1]} vs {k.shape[1]}"
        assert q.shape[-1] == k.shape[-1], f"Feature dimension mismatch: {q.shape[-1]} vs {k.shape[-1]}"
        # before einsum
        ...
        assert k.is_contiguous(), k.stride()
        assert not q._base is k, "q k share storage!"
        # keep dtype consistent with scale
        # # 在报错前插入
        # print(f"[DEBUG] q.shape={q.shape}, k.shape={k.shape}, q.dtype={q.dtype}, q.device={q.device},q.stride={q.stride},k.dtype={k.dtype}, k.device={k.device}, k.stride={k.stride}, scale={self.scale}")
        
        # print(f"[DEBUG] q.shape={q.shape}, k.shape={k.shape}, q.dtype={q.dtype}, q.device={q.device},q.stride={q.stride},k.dtype={k.dtype}, k.device={k.device}, k.stride={k.stride}, scale={self.scale},q_scale={self.q_scale}, k_scale={self.k_scale}")
        ...
        # 检测attn和v的shape是否正确，并检查是否存在NAN,inf的情况
        # Debug checks for shape and NaN/inf values
        ...
        if torch.isnan(v).any() or torch.isinf(v).any():
            print(f"Warning: v contains NaN or inf values, shape: {v.shape}")
        # Ensure proper shapes for einsum operation
        assert attn.shape[0] == v.shape[0], f"Batch size mismatch: {attn.shape[0]} vs {v.shape[0]}"
        assert attn.shape[1] == v.shape[1], f"Head count mismatch: {attn.shape[1]} vs {v.shape[1]}"
        assert attn.shape[-1] == v.shape[-2], f"Attention dimension mismatch: {attn.shape[-1]} vs {v.shape[-2]}"
