...
class CTClipTrainer(nn.Module):
    ...
    # def train_step(self):
    #     torch.autograd.set_detect_anomaly(True)  # 7.24 增加的一部分，无bug的时候试试删除
    #     device = self.device
    ...
    #     # 有两种方案，一个是每个step同时做2D和3D的训练。另一个是每个step只做2D或3D的训练，但是要用到梯度累积。
    #     for dataset_name in self.dataset_names:
    #         print('rank', torch.distributed.get_rank(),'dataset_name', dataset_name)
    ...
    #         with torch.autograd.detect_anomaly(): # 增加的这一行，反应bwckward报错信息
