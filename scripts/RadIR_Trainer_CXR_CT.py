...
from uncon_dataset import ImageReportDataset
from conditional_dataset_train import Conditional_ImageReportDataset_Train, collate_fn
from conditional_dataset_evaluate import Conditional_ImageReportDataset_Eval, custom_data_split
        ...
        *,
        ...
        if self.stage1:
            ...
            for i in range(len(uncon_dataset_names)):
                ...
                # self.uncon_dss[uncon_dataset_name] = ImageReportDataset(os.path.join(uncon_data_train_jsonl[i],train_split_name + '.jsonl'),need_aug=True,modality=uncon_modality[i],is_train=True)
                if not train_no_aug:
                    self.uncon_dss[uncon_dataset_name] = ImageReportDataset(os.path.join(uncon_data_train_jsonl[i],train_split_name + '.jsonl'),need_aug=True,modality=uncon_modality[i],is_train=True,modal_embedding=modal_embedding)
                else:
                    self.uncon_dss[uncon_dataset_name] = ImageReportDataset(os.path.join(uncon_data_train_jsonl[i],train_split_name + '.jsonl'),need_aug=False,modality=uncon_modality[i],is_train=True,modal_embedding=modal_embedding)        
                    
                self.uncon_valid_dss[uncon_dataset_name] = ImageReportDataset(os.path.join(uncon_data_valid_jsonl[i], uncon_valid[i] + '.jsonl'),need_aug=False,modality=uncon_modality[i],is_train=False,modal_embedding=modal_embedding)
                self.uncon_train_valid_dss[uncon_dataset_name] = ImageReportDataset(os.path.join(uncon_data_train_jsonl[i],uncon_train_filter[i][0] + '.jsonl'),need_aug=False,modality=uncon_modality[i],is_train=False,modal_embedding=modal_embedding,length_=train_valid_length_)  # 用于计算soft label的相似度
                # 创建数据加载器
        ...
        if self.stage2:
            ...
            for i in range(len(dataset_names)):
                ...
                with open(f"/dataset_distribution/{dataset_name}_anatomy_distribution.json", 'r') as f:
                    anatomy_distribution = json.load(f)
                anatomy_samples = {anatomy:anatomy_distribution[anatomy]["nums"] for anatomy in anatomy_filter[i]}   # pos 样本越多，采样越多
                ...
                if not train_no_aug:
                    ds_i = Conditional_ImageReportDataset_Train(
                ...
                valid_ds_i = Conditional_ImageReportDataset_Eval(
                    modality=modality_type,
        ...
        with torch.no_grad():
            ...
            for dataset_name in self.dataset_names:
                ...
                if not self.modal_embedding:
                    ...
                    for video, anatomy_ls, sample_id_ls,modal_indexs in tqdm(self.valid_dls[dataset_name], desc=f"Rank {torch.distributed.get_rank()}"):
                        # video: B 1 d h w
                        video = video.to(self.device).unsqueeze(1)  # B 1 1 d h w (凑出local_B)
                        anatomy_ls = list(anatomy_ls)
                        sample_id_ls = list(sample_id_ls)
            ...
            with self.accelerator.accumulate(self.Rad_IR):
                ...
                if stage == 'stage2':
                    ...
                    if not self.modal_embedding: 
                        ...
                        video, similarity_tab, anatomy, _, modal_indexs = next(dl_iter) # 选择一个元素即可
                    # 直接半精度进 GPU，减少显存与带宽
                    tgt_dtype = torch.float16 if mp == "fp16" else (
                                torch.bfloat16 if mp == "bf16" else torch.float32)
